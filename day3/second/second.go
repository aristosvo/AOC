...
func main() {
	...
	for i := 1; i < len(coordinatesRed); i++ {
		for j := 1; j < len(coordinatesBlue); j++ {
			...
			distanceRed := distanceCovered(append(coordinatesRed[:i], coordinate))
			distanceBlue := distanceCovered(append(coordinatesBlue[:j], coordinate))
			distanceToCrossing := distanceRed + distanceBlue
			if leastDistanceToCrossing < distanceToCrossing && leastDistanceToCrossing != 0 {
				continue
			}
			leastDistanceToCrossing = distanceToCrossing
		}
	}
	fmt.Println(leastDistanceToCrossing)
}

func distanceCovered(c []Coordinate) int {
	sum := 0
	for i := 1; i < len(c); i++ {
		sum += distanceBetweenCoordinates(c[i-1], c[i])
	}
	return sum
}

func distanceBetweenCoordinates(c1, c2 Coordinate) int {
